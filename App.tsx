...
function McpScreen({ rest }: { rest: HermesRestClient | null }) {
  const [servers,setServers]=useState<McpServer[]>([]); const [catalog,setCatalog]=useState<any[]>([]); const [err,setErr]=useState(''); const [loading,setLoading]=useState(false);
  const load=async()=>{ if(!rest)return; setLoading(true); try { const [s,c]=await Promise.all([rest.mcpServers(), rest.mcpCatalog().catch(()=>[]) as any]); setServers(Array.isArray(s)?s:[]); setCatalog(Array.isArray(c)?c:(c.servers||c.items||[])); } finally { setLoading(false); } };
  useEffect(()=>{ load().catch(e=>setErr(e.message));},[rest]);
  return <ScrollView contentContainerStyle={styles.screen}><Card><Text style={styles.sectionTitle}>MCP setup</Text><Text style={styles.muted}>Inspect configured MCP servers and browse catalog entries.</Text>{err?<Text style={styles.error}>{err}</Text>:null}<Button text="Refresh" icon="refresh-outline" secondary onPress={load}/></Card>{loading ? <LoadingBlock label="Loading MCP…" /> : <><Card><Text style={styles.sectionTitle}>Servers</Text>{servers.map(s=><ToggleRow key={s.name} title={safeText(s.name)} sub={safeText(s.command||s.url||s.status||'')} on={!!s.enabled} onPress={async()=>{await rest?.setMcpEnabled(s.name,!s.enabled); await load();}} />)}</Card><Card><Text style={styles.sectionTitle}>Catalog</Text>{catalog.slice(0,20).map((c:any,i:number)=><View key={c.name||i} style={styles.listItem}><View style={{ flex: 1 }}><Text style={styles.listTitle}>{safeText(c.name||c.id)}</Text><Text style={styles.listSub}>{safeText(c.description||c.command||'')}</Text></View></View>)}</Card></>}</ScrollView>;
}
