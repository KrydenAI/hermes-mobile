  ...
  return (
    <SafeAreaProvider>
      <SafeAreaView style={styles.safe} edges={['top', 'bottom', 'left', 'right']}>
        <LinearGradient colors={[colors.bg, '#eefafa', '#ffffff']} style={StyleSheet.absoluteFill} />
...
function TabBar({ active, setActive }: { active: Exclude<Tab, 'home'>; setActive: (t: Exclude<Tab, 'home'>) => void }) {
  return <View style={styles.tabs}>
    {tabs.map(t => <Pressable key={t.id} onPress={() => setActive(t.id)} style={[styles.tab, active === t.id && styles.tabActive]}>
      <Ionicons name={t.icon} size={20} color={active === t.id ? colors.primary : colors.muted} />
...
function ConnectScreen({ profiles, active, status, error, statusPayload, onSelect, onSave, onDelete }: any) {
  ...
  return <ScrollView contentContainerStyle={styles.onboardingScreen}>
    <View style={styles.hero}>
      ...
      <View style={styles.orbital}><View style={styles.orbitalRing} /><View style={[styles.orbitalRing, styles.orbitalRingTilt]} /><LinearGradient colors={[colors.primary2, colors.primary]} style={styles.orbitalCore}><Ionicons name="sparkles" size={30} color="#ffffff" /></LinearGradient></View>
...
function SettingsScreen({ rest, profile, statusPayload }: { rest: HermesRestClient | null; profile: ConnectionProfile | null; statusPayload: any }) {
  const [model,setModel]=useState<any>(null); const [logs,setLogs]=useState<any>(null); const [loading,setLoading]=useState(false);
  const load=async()=>{ if(!rest)return; setLoading(true); try { const [m,l]=await Promise.all([rest.modelInfo().catch((e:any)=>({error:e.message})), rest.logs(40).catch((e:any)=>({error:e.message}))]); setModel(m); setLogs(l); } finally { setLoading(false); } };
  useEffect(()=>{load().catch(()=>undefined)},[rest]);
  return <ScrollView contentContainerStyle={styles.screen}>{loading ? <LoadingBlock label="Loading system info…" /> : null}<JsonCard title="Connection details" data={{ name: profile?.name, baseUrl: profile?.baseUrl, platform: Platform.OS }} /><ProfileCopyPanel rest={rest} /><JsonCard title="Auth + session" data={{ authMode: profile?.authMode || 'auto', token: profile?.token ? '[stored securely]' : null, username: profile?.username || null, password: profile?.password ? '[stored securely]' : null }} /><JsonCard title="System status" data={statusPayload}/><JsonCard title="Raw JSON / status output" data={{ model, logs }}/><Card><Text style={styles.sectionTitle}>Advanced / admin</Text><Text style={styles.muted}>Dangerous actions stay behind explicit details/overflow controls.</Text></Card></ScrollView>;
}

function ProfileCopyPanel({ rest }: { rest: HermesRestClient | null }) {
  const [profiles,setProfiles]=useState<any[]>([]);
  const [source,setSource]=useState('default');
  const [newName,setNewName]=useState('founder-agent');
  const [installSource,setInstallSource]=useState('https://github.com/NousResearch/hermes-agent-profile');
  const [includeAlias,setIncludeAlias]=useState(true);
  const [cloneAll,setCloneAll]=useState(true);
  const [loading,setLoading]=useState(false);
  const [err,setErr]=useState('');
  const [notice,setNotice]=useState('');
  const normalizeProfiles=(raw:any):any[]=>Array.isArray(raw)?raw:(Array.isArray(raw?.profiles)?raw.profiles:[]);
  const load=async()=>{ if(!rest)return; setLoading(true); setErr(''); try { const rows=normalizeProfiles(await rest.profiles()); setProfiles(rows); if(rows.length && !rows.some((p:any)=>p.name===source)) setSource(safeText(rows[0].name||'default')); } catch(e:any){ setErr(e.message); } finally { setLoading(false); } };
  useEffect(()=>{ load().catch(e=>setErr(e.message)); },[rest]);
  const installCommand=()=>`hermes profile install ${installSource.trim() || '<source>'}${newName.trim()?` --name ${newName.trim()}`:''}${includeAlias?' --alias':''}`;
  const copyInstallCommand=async()=>{ await Clipboard.setStringAsync(installCommand()); setNotice('Copied install command with --alias.'); Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(()=>undefined); };
  const copyExistingProfile=async()=>{ if(!rest)return; const name=newName.trim(); if(!name){setErr('New profile name is required.'); return;} setErr(''); setNotice(''); try { const result=await rest.createProfile({ name, clone_from: source, clone_all: cloneAll, description: `Copied from ${source} in Hermes Mobile` }); const setup=await rest.profileSetupCommand(name).catch(()=>({ command: name === 'default' ? 'hermes setup' : `${name} setup` })); setNotice(`Created ${safeText(result?.name||name)}${includeAlias?' with alias wrapper when available':''}. Setup: ${safeText(setup.command)}`); await Clipboard.setStringAsync(includeAlias ? safeText(setup.command) : `hermes -p ${name}`); await load(); } catch(e:any){ setErr(e.message); Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error).catch(()=>undefined); } };
  return <Card>
    <View style={styles.rowBetween}><Text style={styles.sectionTitle}>Profiles</Text><Text style={styles.listSub}>{profiles.length} installed</Text></View>
    <Text style={styles.muted}>Copy a local profile into a new alias-ready profile, or copy the one-line distribution install command for sharing.</Text>
    {err?<Text style={styles.error}>{err}</Text>:null}{notice?<Text style={styles.success}>{notice}</Text>:null}
    {loading ? <LoadingBlock label="Loading profiles…" compact /> : profiles.map((p:any)=><View key={safeText(p.name)} style={styles.compactListItem}><View style={styles.compactMain}><View style={{ flex: 1 }}><Text style={styles.listTitle}>{safeText(p.name)}</Text><Text style={styles.listSub} numberOfLines={1}>{safeText(p.provider||'provider?')} · {safeText(p.model||'model?')} · {p.has_alias?'alias':'no alias'}</Text></View><Pressable onPress={()=>setSource(safeText(p.name))} style={[styles.miniPill, source===safeText(p.name)&&styles.miniPillActive]}><Text style={styles.miniPillText}>{source===safeText(p.name)?'Source':'Use'}</Text></Pressable></View></View>)}
    <Label text="Copy from"/><Input value={source} onChangeText={setSource} placeholder="default" autoCapitalize="none" />
    <Label text="New profile / alias"/><Input value={newName} onChangeText={setNewName} placeholder="founder-agent" autoCapitalize="none" />
    <View style={styles.row}><Pressable onPress={()=>setCloneAll(v=>!v)} style={[styles.drawerFilterPill, cloneAll && styles.drawerFilterPillActive]}><Text style={styles.drawerFilterText}>{cloneAll?'Full copy':'Config only'}</Text></Pressable><Pressable onPress={()=>setIncludeAlias(v=>!v)} style={[styles.drawerFilterPill, includeAlias && styles.drawerFilterPillActive]}><Text style={styles.drawerFilterText}>{includeAlias?'--alias on':'--alias off'}</Text></Pressable></View>
    <Button text="Copy profile + alias" icon="copy-outline" onPress={copyExistingProfile} />
    <View style={styles.divider} />
    <Label text="Distribution source"/><Input value={installSource} onChangeText={setInstallSource} placeholder="https://github.com/org/hermes-profile" autoCapitalize="none" />
    <Command text={installCommand()} />
    <Button text="Copy install --alias command" icon="terminal-outline" secondary onPress={copyInstallCommand} />
  </Card>;
}
...
const styles = StyleSheet.create({
  ...
  pill: { flexDirection:'row', gap:8, alignItems:'center', borderWidth:1, borderColor: colors.stroke, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 7, backgroundColor:'rgba(255,255,255,0.82)', maxWidth: 170 }, pillGood:{borderColor:'rgba(5,150,105,0.36)'}, pillBad:{borderColor:'rgba(225,29,72,0.34)'}, pillText:{color:colors.text,fontSize:12,fontWeight:'700'}, dot:{width:8,height:8,borderRadius:99},
  screen: { padding: 14, paddingBottom: 170, gap: 12 },
  card: { backgroundColor:colors.card, borderWidth:1, borderColor: colors.stroke, borderRadius: radius.lg, padding:14, gap:10, ...shadow }, sectionTitle:{color:colors.text,fontSize:20,fontWeight:'800',letterSpacing:-0.4}, muted:{color:colors.muted,lineHeight:21}, mutedCenter:{color:colors.muted,lineHeight:21,textAlign:'center'}, text:{color:colors.text,lineHeight:21}, error:{color:colors.bad,fontWeight:'700'}, errorInline:{color:colors.bad,fontWeight:'700',paddingHorizontal:14,paddingBottom:6}, success:{color:colors.good,fontWeight:'700'}, label:{color:colors.muted,fontSize:12,fontWeight:'800',textTransform:'uppercase',letterSpacing:0.8},
  input:{backgroundColor:'#ffffff', borderWidth:1, borderColor:colors.stroke, borderRadius:radius.md, paddingHorizontal:14, paddingVertical:12, color:colors.text, fontSize:15},
  row:{flexDirection:'row', gap:10, flexWrap:'wrap'}, rowBetween:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',gap:12}, button:{backgroundColor:colors.primary, paddingHorizontal:14, paddingVertical:12, borderRadius:radius.md, flexDirection:'row',alignItems:'center',gap:7, justifyContent:'center'}, buttonSecondary:{backgroundColor:'#ffffff', borderWidth:1,borderColor:colors.stroke}, buttonText:{color:'#ffffff',fontWeight:'800'}, buttonSecondaryText:{color:colors.primary}, divider:{height:1,backgroundColor:colors.stroke,marginVertical:4}, miniPill:{borderWidth:1,borderColor:colors.stroke,borderRadius:999,paddingHorizontal:10,paddingVertical:7,backgroundColor:'#ffffff'}, miniPillActive:{backgroundColor:'#ccfbf1',borderColor:colors.primary2}, miniPillText:{color:colors.primary,fontSize:12,fontWeight:'900'},
  code:{fontFamily:Platform.select({ios:'Menlo',android:'monospace',default:'monospace'}), color:'#134e4a', backgroundColor:'#ecfeff', borderWidth:1, borderColor:colors.stroke, borderRadius:radius.md, padding:12, overflow:'hidden', lineHeight:19},
  listItem:{borderWidth:1,borderColor:colors.stroke,backgroundColor:'#ffffff',borderRadius:radius.md,padding:13,flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:12}, selected:{borderColor:colors.primary2,backgroundColor:'#ccfbf1'}, listTitle:{color:colors.text,fontSize:16,fontWeight:'800'}, listSub:{color:colors.muted,fontSize:12,marginTop:3},
  tabs:{height:78,borderTopWidth:1,borderColor:colors.stroke,backgroundColor:'rgba(255,255,255,0.96)',flexDirection:'row',paddingHorizontal:8,paddingTop:8,paddingBottom:10,gap:4}, tabsInner:{padding:10,gap:8}, tab:{flex:1,alignItems:'center',justifyContent:'center',gap:3,paddingHorizontal:2,paddingVertical:7,borderRadius:radius.md,borderWidth:1,borderColor:'transparent'}, tabActive:{backgroundColor:'#ccfbf1',borderColor:'rgba(20,184,166,0.42)'}, tabText:{color:colors.muted,fontSize:11,fontWeight:'800'},
  toggle:{width:48,height:28,borderRadius:999,backgroundColor:'#dbeafe',padding:3}, toggleOn:{backgroundColor:colors.primary}, knob:{width:22,height:22,borderRadius:99,backgroundColor:'#ffffff'}, knobOn:{transform:[{translateX:20}]},
  loading:{borderWidth:1,borderColor:colors.stroke,backgroundColor:'#ffffff',borderRadius:radius.lg,padding:20,gap:10,alignItems:'center',justifyContent:'center'},
  chatRoot:{flex:1}, chatTopBar:{flexDirection:'row',alignItems:'center',gap:10,paddingHorizontal:12,paddingVertical:10,borderBottomWidth:1,borderColor:colors.stroke,backgroundColor:'rgba(255,255,255,0.76)'}, iconButton:{width:42,height:42,borderRadius:99,alignItems:'center',justifyContent:'center',backgroundColor:'#ffffff',borderWidth:1,borderColor:colors.stroke}, chatTitle:{color:colors.text,fontSize:18,fontWeight:'900'}, chatSub:{color:colors.muted,fontSize:12,marginTop:2}, chatMessages:{padding:14,paddingBottom:28,gap:8,minHeight:'100%'}, centerPane:{flex:1,minHeight:420,alignItems:'center',justifyContent:'center',gap:10,paddingHorizontal:30},
  composer:{flexDirection:'row',alignItems:'flex-end',gap:10,padding:12,borderTopWidth:1,borderColor:colors.stroke,backgroundColor:'rgba(255,255,255,0.96)'}, composerInput:{flex:1,maxHeight:130,backgroundColor:'#ffffff',borderWidth:1,borderColor:colors.stroke,borderRadius:22,paddingHorizontal:15,paddingVertical:11,color:colors.text}, send:{backgroundColor:colors.primary,borderRadius:99,width:46,height:46,alignItems:'center',justifyContent:'center'},
  messageWrap:{alignSelf:'stretch',gap:6,marginBottom:8}, messageWrapUser:{alignItems:'flex-end'}, bubble:{borderRadius:radius.lg,padding:13,gap:5}, userBubble:{backgroundColor:'#ccfbf1',alignSelf:'flex-end',maxWidth:'92%'}, assistantBubble:{backgroundColor:'#ffffff',alignSelf:'flex-start',maxWidth:'96%',borderWidth:1,borderColor:colors.stroke}, bubbleRole:{color:colors.primary2,fontSize:11,fontWeight:'900',textTransform:'uppercase'},
  drawerScrim:{...StyleSheet.absoluteFillObject,backgroundColor:'rgba(15,23,42,0.25)'}, drawer:{position:'absolute',left:0,top:0,bottom:0,width:'82%',maxWidth:360,backgroundColor:'#f8ffff',borderRightWidth:1,borderColor:colors.stroke,paddingTop:58,paddingHorizontal:14,gap:12}, drawerItem:{borderWidth:1,borderColor:colors.stroke,backgroundColor:'#ffffff',borderRadius:radius.md,padding:13}, drawerFilterRow:{flexDirection:'row',gap:10}, drawerFilterPill:{flex:1,borderWidth:1,borderColor:colors.stroke,borderRadius:radius.md,paddingVertical:10,alignItems:'center',justifyContent:'center',backgroundColor:'#ffffff'}, drawerFilterPillActive:{backgroundColor:'#ccfbf1',borderColor:colors.primary2}, drawerFilterText:{color:colors.text,fontSize:13,fontWeight:'900'}, pulseFilterRow:{flexDirection:'row',gap:7,flexWrap:'wrap',marginTop:4}, pulseFilterPill:{borderWidth:1,borderColor:colors.stroke,borderRadius:999,paddingHorizontal:11,paddingVertical:8,backgroundColor:'#ffffff'}, pulseFilterPillActive:{backgroundColor:'#ccfbf1',borderColor:colors.primary2},
  toolCard:{alignSelf:'flex-start',maxWidth:'96%',borderWidth:1,borderColor:'rgba(15,118,110,0.24)',backgroundColor:'#ecfeff',borderRadius:radius.md,padding:10,flexDirection:'row',gap:10,alignItems:'flex-start'}, skillCard:{borderColor:'rgba(20,184,166,0.28)',backgroundColor:'#f0fdfa'}, toolCardError:{borderColor:'rgba(225,29,72,0.30)',backgroundColor:'#fff1f2'}, toolGlyph:{width:25,height:25,borderRadius:99,alignItems:'center',justifyContent:'center',backgroundColor:'#ffffff'}, toolTitle:{color:colors.text,fontSize:13,fontWeight:'900'}, toolSub:{color:colors.muted,fontSize:12,lineHeight:17,marginTop:2}, toolDetail:{color:colors.bad,fontSize:12,lineHeight:17,marginTop:6,fontFamily:Platform.select({ios:'Menlo',android:'monospace',default:'monospace'})}, activityLine:{alignSelf:'flex-start',color:colors.muted,fontSize:12,fontWeight:'700',paddingHorizontal:3,paddingVertical:2},
  headerCompact:{paddingHorizontal:16,paddingTop:8,paddingBottom:10,flexDirection:'row',alignItems:'center',justifyContent:'space-between',borderBottomWidth:1,borderColor:colors.stroke,backgroundColor:'rgba(255,255,255,0.78)'}, rowCenter:{flexDirection:'row',alignItems:'center',gap:10,flex:1}, headerBack:{width:34,height:34,borderRadius:99,alignItems:'center',justifyContent:'center',backgroundColor:'#ffffff',borderWidth:1,borderColor:colors.stroke}, headerTitle:{color:colors.text,fontSize:20,fontWeight:'900',letterSpacing:-0.3}, headerSub:{color:colors.muted,fontSize:12,marginTop:2},
  onboardingScreen:{padding:16,paddingBottom:36,gap:14}, hero:{alignItems:'center',gap:10,paddingTop:22,paddingBottom:8,paddingHorizontal:8}, heroTitle:{color:colors.text,fontSize:42,fontWeight:'900',letterSpacing:-1.4,textAlign:'center'}, orbital:{width:148,height:132,alignItems:'center',justifyContent:'center',marginVertical:8}, orbitalRing:{position:'absolute',width:142,height:76,borderRadius:999,borderWidth:1,borderColor:'rgba(15,118,110,0.46)',transform:[{rotate:'-18deg'}]}, orbitalRingTilt:{borderColor:'rgba(20,184,166,0.34)',transform:[{rotate:'24deg'}]}, orbitalCore:{width:70,height:70,borderRadius:35,alignItems:'center',justifyContent:'center',shadowColor:colors.primary,shadowOpacity:0.35,shadowRadius:18}, advancedLink:{alignSelf:'center',flexDirection:'row',alignItems:'center',gap:5,paddingVertical:4}, advancedText:{color:colors.primary,fontWeight:'800'}, advancedPanel:{gap:10,borderTopWidth:1,borderColor:colors.stroke,paddingTop:10}, compactIcon:{width:34,height:34,borderRadius:99,alignItems:'center',justifyContent:'center',backgroundColor:'#ffffff',borderWidth:1,borderColor:colors.stroke},
  opsRow:{borderWidth:1,borderColor:colors.stroke,backgroundColor:'#ffffff',borderRadius:radius.lg,padding:15,flexDirection:'row',alignItems:'center',gap:12,...shadow}, opsIcon:{width:42,height:42,borderRadius:14,alignItems:'center',justifyContent:'center',backgroundColor:'#ccfbf1',borderWidth:1,borderColor:'rgba(20,184,166,0.34)'}, compactListItem:{borderWidth:1,borderColor:colors.stroke,backgroundColor:'#ffffff',borderRadius:radius.md,overflow:'hidden'}, compactMain:{padding:12,flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:10}, statusDot:{width:9,height:9,borderRadius:99,backgroundColor:colors.good}, statusDotOff:{backgroundColor:colors.muted}, detailsPanel:{borderTopWidth:1,borderColor:colors.stroke,padding:12,gap:10,backgroundColor:'#f8ffff'}, briefModal:{position:'absolute',left:12,right:12,top:74,bottom:22,backgroundColor:'#ffffff',borderWidth:1,borderColor:colors.stroke,borderRadius:radius.lg,padding:14,gap:12,...shadow}
});
